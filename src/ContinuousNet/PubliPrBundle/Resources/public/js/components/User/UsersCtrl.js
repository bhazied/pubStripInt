...
function($scope, $rootScope, $sce, $timeout, $filter, ngTableParams, $state, $q, $interpolate, $localStorage, toaster, SweetAlert, $companiesDataFactory, $countriesDataFactory, $languagesDataFactory, $usersDataFactory) {
    ...
    $scope.setCols = function() {
        $scope.cols = [
            ...
            { field: 'company', title: $filter('translate')('content.list.fields.COMPANY'), sortable: 'company.name', filter: { 'user.company': 'select' }, getValue: $scope.linkValue, filterData: $scope.getCompanies(), show: $scope.getParamValue('company_id_show_filed', false), displayField: 'name', state: 'app.nogroup.companiesdetails' },
