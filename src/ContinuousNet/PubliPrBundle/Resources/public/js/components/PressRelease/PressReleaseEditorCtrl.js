...
app.controller('PressReleaseEditorCtrl', ['$scope', '$state', '$stateParams', '$sce', '$timeout', '$filter', '$uibModal', '$q', '$interpolate', '$localStorage', 'toaster', 'SweetAlert', 'savable', '$newsroomsDataFactory', '$usersDataFactory', '$pressReleasesDataFactory', '$blockContentsDataFactory',
    function($scope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $newsroomsDataFactory, $usersDataFactory, $pressReleasesDataFactory, $blockContentsDataFactory) {
        ...
        $scope.blockContents = [];
        $scope.blockContentsLoaded = false;

        $scope.getBlockContents = function() {
            $timeout(function(){
                $scope.blockContentsLoaded = true;
                if ($scope.blockContents.length == 0) {
                    $scope.blockContents.push({});
                    var def = $q.defer();
                    $blockContentsDataFactory.query({offset: 0, limit: 10000, 'order_by[blockContent.id]': 'desc'}).$promise.then(function(data) {
                        for (var i in data.results) {
                            data.results[i].hidden = false;
                        }
                        $scope.blockContents = data.results;
                        def.resolve($scope.blockContents);
                    });
                    return def;
                } else {
                    return $scope.blockContents;
                }
            });
        };

        $scope.getBlockContents();

        $scope.showBlockContents = flase;
        $scope.addContentBlock = function() {
            $scope.showBlockContents = !$scope.showBlockContents;
        };
