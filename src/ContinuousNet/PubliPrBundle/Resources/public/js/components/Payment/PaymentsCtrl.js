...
function($scope, $rootScope, $sce, $timeout, $filter, ngTableParams, $state, $q, $interpolate, $localStorage, toaster, SweetAlert, $usersDataFactory, $paymentsDataFactory) {
    ...
    $scope.setCols = function() {
        $scope.cols = [
            ...
            { field: 'is_valid', title: $filter('translate')('content.list.fields.ISVALID'), sortable: 'payment.isValid', filter: { 'payment.isValid': 'select' }, show: $scope.getParamValue('is_valid_show_filed', true), getValue: $scope.interpolatedValue, filterData : $scope.booleanOptions, interpolateExpr: $interpolate('<span my-boolean="[[ row.is_valid ]]"></span>') },
