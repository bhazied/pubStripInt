...
/**
 * Controller for Newsrooms Users Form
 */

app.controller('NewsroomsUsersFormCtrl', ['$scope', '$state', '$stateParams', '$sce', '$timeout', '$filter', '$uibModal', '$q', '$interpolate', '$localStorage', 'toaster', 'SweetAlert', 'savable', '$usersDataFactory', '$newsroomsUsersDataFactory',
function($scope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $usersDataFactory, $newsroomsUsersDataFactory) {
    ...
    if (angular.isDefined($stateParams.id)) {
        $newsroomsUsersDataFactory.get({id: $stateParams.id}).$promise.then(function(data) {
            $timeout(function(){
                $scope.newsroomsUsers = savable(data);
                //console.warn($scope.newsroomsUsers);
            });
        });
    } else {
        $scope.newsroomsUsers = {id: 0};
