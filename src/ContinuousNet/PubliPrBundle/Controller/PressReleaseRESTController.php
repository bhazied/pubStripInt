 ...
 * @see      PressReleaseRESTController
 * @since      Class available since Release 1.0
 * @deprecated Nothing
 * @access    public
 * @RouteResource("PressRelease")
 */
class PressReleaseRESTController extends BaseRESTController
{
    /**
     * Get a Press Release entity
     *
    ...
    /**
     * Get all Press Release entities.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->leftJoin('ContinuousNet\PubliPrBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'pr_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\PubliPrBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'pr_.modifierUser = modifier_user.id');
            $textFields = array('pressRelease.title', 'pressRelease.slug', 'pressRelease.description', 'pressRelease.picturePreview', 'pressRelease.content', 'pressRelease.metaTitle', 'pressRelease.metaDescription', 'pressRelease.metaKeywords');
            foreach ($filters as $field => $value) {
                $_field = str_replace('pressRelease.', 'pr_.', $field);
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('pressRelease.', 'pr_.', $field);
                $qbList->addOrderBy($field, $direction);
            }
            $qbList->select('pr_');
            $qbList->setMaxResults($limit);
            $qbList->setFirstResult($offset);
            $qbList->groupBy('pr_.id');
    ...
    /**
     * Partial Update to a Press Release entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
